...
function Accordion({ title, emoji, isOpen, onToggle, children, badge }: AccordionProps) {
  return (
...
function ScenarioSummary({ state, currentYearData, effectiveGrowthRate, effectiveInterestRate }: ScenarioSummaryProps) {
  const totalImmigration = state.workBasedImmigration + state.familyImmigration + state.humanitarianImmigration;
  
  return (
    <div className="px-4 py-3 bg-gradient-to-r from-amber-950/30 to-transparent border-b border-gray-800">
...
export function ScenarioPanel({ state, actions, currentYearData, effectiveGrowthRate, effectiveInterestRate }: ScenarioPanelProps) {
  ...
  return (
    <aside className="w-80 flex-shrink-0 bg-gray-900/50 border-r border-gray-800 overflow-y-auto">
      ...
      <div className="p-4 text-xs text-gray-600">
        <p>Simulation: 1990–2060</p>
        <p>Model includes birth rates, mortality, immigration fiscal impact, GDP growth, and debt accumulation.</p>
      </div>
    </aside>
  );
}
